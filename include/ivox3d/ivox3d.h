...
#ifndef ww_lslam_IVOX3D_H
#define ww_lslam_IVOX3D_H
...
class IVox {
   public:
    ...
    void GetAllPoints(PointVector& pts);
...
void IVox<dim, node_type, PointType>::GetAllPoints(PointVector& pts) {
    for (auto it = grids_map_.begin(); it != grids_map_.end(); ++it) {
        ...
        PointType pt;
        pt.x = key(0, 0) * options_.resolution_;
        pt.y = key(1, 0) * options_.resolution_;
        pt.z = key(2, 0) * options_.resolution_;
        pts.push_back(pt);
    }
}
